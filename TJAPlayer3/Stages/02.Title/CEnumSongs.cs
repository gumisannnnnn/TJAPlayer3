...
namespace TJAPlayer4
{
	...
	{													// ファイルキャッシュ(songslist.db)からの取得と、ディスクからの取得を、この一つのクラスに集約。
		...
		private readonly string strPathSongsDB = TJAPlayer4.strEXEのあるフォルダ + "songs.db";
		private readonly string strPathSongList = TJAPlayer4.strEXEのあるフォルダ + "songlist.db";
		...
		public void t曲リストの構築1()
		{
			...
			try
			{
				...
				//-----------------------------
				TJAPlayer4.stage起動.eフェーズID = CStage.Eフェーズ.起動0_システムサウンドを構築;
				...
				try
				{
					TJAPlayer4.Skin.bgm起動画面.t再生する();
					for ( int i = 0; i < TJAPlayer4.Skin.nシステムサウンド数; i++ )
					{
						if ( !TJAPlayer4.Skin[ i ].b排他 )	// BGM系以外のみ読み込む。(BGM系は必要になったときに読み込む)
						{
							CSkin.Cシステムサウンド cシステムサウンド = TJAPlayer4.Skin[ i ];
					...
					lock ( TJAPlayer4.stage起動.list進行文字列 )
					{
						TJAPlayer4.stage起動.list進行文字列.Add( "SYSTEM SOUND...OK" );
					}
				}
				finally
				{
				...
				//-----------------------------
				TJAPlayer4.stage起動.eフェーズID = CStage.Eフェーズ.起動00_songlistから曲リストを作成する;
				...
				try
				{
					if ( !TJAPlayer4.ConfigIni.bConfigIniがないかDTXManiaのバージョンが異なる )
					{
						...
						lock ( TJAPlayer4.stage起動.list進行文字列 )
						{
							TJAPlayer4.stage起動.list進行文字列.Add( "SONG LIST...OK" );
						}
					}
					else
					{
						...
						lock ( TJAPlayer4.stage起動.list進行文字列 )
						{
							TJAPlayer4.stage起動.list進行文字列.Add( "SONG LIST...SKIPPED" );
						}
					}
				}
				finally
				...
				//-----------------------------
				TJAPlayer4.stage起動.eフェーズID = CStage.Eフェーズ.起動1_SongsDBからスコアキャッシュを構築;
				...
				try
				{
					if ( !TJAPlayer4.ConfigIni.bConfigIniがないかDTXManiaのバージョンが異なる )
					{
						...
						lock ( TJAPlayer4.stage起動.list進行文字列 )
						{
							TJAPlayer4.stage起動.list進行文字列.Add( "SONG DATABASE...OK" );
						}
					}
					else
					{
						...
						lock ( TJAPlayer4.stage起動.list進行文字列 )
						{
							TJAPlayer4.stage起動.list進行文字列.Add( "SONG DATABASE...SKIPPED" );
						}
					}
				}
				finally
			...
			{
				TJAPlayer4.stage起動.eフェーズID = CStage.Eフェーズ.起動7_完了;
		...
		private void t曲リストの構築2()
		{
			...
			try
			{
				...
				try
				{
					if ( !string.IsNullOrEmpty( TJAPlayer4.ConfigIni.str曲データ検索パス ) )
					{
						string[] strArray = TJAPlayer4.ConfigIni.str曲データ検索パス.Split( new char[] { ';' } );
						if ( strArray.Length > 0 )
						{
							...
							foreach ( string str in strArray )
							{
								...
								{
									path = TJAPlayer4.strEXEのあるフォルダ + str;	// 相対パスの場合、絶対パスに直す(2010.9.16)
								}
