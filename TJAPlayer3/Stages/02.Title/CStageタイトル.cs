	...
	internal class CStageタイトル : CStage
	{
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
				...
				if( base.b初めての進行描画 )
				{
					if( TJAPlayer4.r直前のステージ == TJAPlayer4.stage起動 )
					{
					...
					this.ctカーソルフラッシュ用.t開始( 0, 700, 5, TJAPlayer4.Timer );
				...
				if( base.eフェーズID == CStage.Eフェーズ.共通_通常状態		// 通常状態、かつ
					&& TJAPlayer4.act現在入力を占有中のプラグイン == null )	// プラグインの入力占有がない
				{
					if( TJAPlayer4.Input管理.Keyboard.bキーが押された( (int) Key.Escape ) )
						return (int) E戻り値.EXIT;

					this.ctキー反復用.Up.tキー反復( TJAPlayer4.Input管理.Keyboard.bキーが押されている( (int)SlimDX.DirectInput.Key.UpArrow ), new CCounter.DGキー処理( this.tカーソルを上へ移動する ) );
					this.ctキー反復用.R.tキー反復( TJAPlayer4.Pad.b押されているGB( Eパッド.HH ), new CCounter.DGキー処理( this.tカーソルを上へ移動する ) );
					if( TJAPlayer4.Pad.b押された( E楽器パート.DRUMS, Eパッド.SD ) )
						this.tカーソルを上へ移動する();

					this.ctキー反復用.Down.tキー反復( TJAPlayer4.Input管理.Keyboard.bキーが押されている( (int)SlimDX.DirectInput.Key.DownArrow ), new CCounter.DGキー処理( this.tカーソルを下へ移動する ) );
					this.ctキー反復用.B.tキー反復( TJAPlayer4.Pad.b押されているGB( Eパッド.BD ), new CCounter.DGキー処理( this.tカーソルを下へ移動する ) );
					if( TJAPlayer4.Pad.b押された( E楽器パート.DRUMS, Eパッド.LT ) )
						this.tカーソルを下へ移動する();

					if( ( TJAPlayer4.Pad.b押されたDGB( Eパッド.CY ) || TJAPlayer4.Pad.b押された( E楽器パート.DRUMS, Eパッド.RD ) ) || ( TJAPlayer4.Pad.b押された( E楽器パート.DRUMS, Eパッド.LC ) || ( TJAPlayer4.ConfigIni.bEnterがキー割り当てのどこにも使用されていない && TJAPlayer4.Input管理.Keyboard.bキーが押された( (int)SlimDX.DirectInput.Key.Return ) ) ) )
					{
						if ( ( this.n現在のカーソル行 == (int) E戻り値.GAMESTART - 1 ) && TJAPlayer4.Skin.soundゲーム開始音.b読み込み成功 )
						{
							TJAPlayer4.Skin.soundゲーム開始音.t再生する();
						}
						else
						{
...
//						Trace.TraceInformation( "DTXMania Title: SPACE key registered. " + CDTXMania.ct.nシステム時刻 );
                ...
                if (TJAPlayer4.Tx.Title_Background != null )
                    TJAPlayer4.Tx.Title_Background.t2D描画( TJAPlayer4.app.Device, 0, 0 );
				...
				if( TJAPlayer4.Tx.Title_Menu != null )
				{
					int x = MENU_X;
					...
					if( this.ctカーソルフラッシュ用.n現在の値 <= 100 )
					{
                        ...
                        TJAPlayer4.Tx.Title_Menu.vc拡大縮小倍率.X = nMag;
                        TJAPlayer4.Tx.Title_Menu.vc拡大縮小倍率.Y = nMag;
                        TJAPlayer4.Tx.Title_Menu.Opacity = (int) ( 255.0 * ( 1.0 - ( ( (double) this.ctカーソルフラッシュ用.n現在の値 ) / 100.0 ) ) );
						int x_magnified = x + ( (int) ( ( MENU_W * ( 1.0 - nMag ) ) / 2.0 ) );
						int y_magnified = y + ( (int) ( ( MENU_H * ( 1.0 - nMag ) ) / 2.0 ) );
                        TJAPlayer4.Tx.Title_Menu.t2D描画( TJAPlayer4.app.Device, x_magnified, y_magnified, new Rectangle( 0, MENU_H * 5, MENU_W, MENU_H ) );
					}
                    TJAPlayer4.Tx.Title_Menu.vc拡大縮小倍率.X = 1f;
                    TJAPlayer4.Tx.Title_Menu.vc拡大縮小倍率.Y = 1f;
                    TJAPlayer4.Tx.Title_Menu.Opacity = 0xff;
                    TJAPlayer4.Tx.Title_Menu.t2D描画( TJAPlayer4.app.Device, x, y, new Rectangle( 0, MENU_H * 4, MENU_W, MENU_H ) );
				}
				if( TJAPlayer4.Tx.Title_Menu != null )
				{
                    ...
                    TJAPlayer4.Tx.Title_Menu.t2D描画( TJAPlayer4.app.Device, MENU_X, MENU_Y, new Rectangle( 0, 0, MENU_W, MENU_H ) );
                    TJAPlayer4.Tx.Title_Menu.t2D描画( TJAPlayer4.app.Device, MENU_X, MENU_Y + MENU_H, new Rectangle( 0, MENU_H * 2, MENU_W, MENU_H * 2 ) );
				}

                // URLの座標が押されたらブラウザで開いてやる 兼 マウスクリックのテスト
                // クライアント領域内のカーソル座標を取得する。
                // point.X、point.Yは負の値になることもある。
                var point = TJAPlayer4.app.Window.PointToClient(System.Windows.Forms.Cursor.Position);
                // クライアント領域の横幅を取得して、1280で割る。もちろんdouble型。
                var scaling = 1.000 * TJAPlayer4.app.Window.ClientSize.Width / 1280;
                if(TJAPlayer4.Input管理.Mouse.bキーが押された((int) MouseObject.Button1))
                {
				...
				switch( eフェーズid )
				{
					case CStage.Eフェーズ.共通_フェードイン:
						...
						{
							TJAPlayer4.Skin.soundタイトル音.t再生する();
...
//			OPTION,
		...
		private void tカーソルを下へ移動する()
		{
			if ( this.n現在のカーソル行 != (int) E戻り値.EXIT - 1 )
			{
				...
				this.n現在のカーソル行++;
				this.ct下移動用.t開始( 0, 100, 1, TJAPlayer4.Timer );
				if( this.ct上移動用.b進行中 )
				{
		...
		private void tカーソルを上へ移動する()
		{
			if ( this.n現在のカーソル行 != (int) E戻り値.GAMESTART - 1 )
			{
				TJAPlayer4.Skin.soundカーソル移動音.t再生する();
				this.n現在のカーソル行--;
				this.ct上移動用.t開始( 0, 100, 1, TJAPlayer4.Timer );
				if( this.ct下移動用.b進行中 )
				{
