	...
	internal class CStageコンフィグ : CStage
	{
		...
		public override void On活性化()
		{
			...
			try
			{
                ...
                {
                    this.ftフォント = new Font(TJAPlayer4.ConfigIni.FontName, 18.0f, FontStyle.Bold, GraphicsUnit.Pixel);
                }
                else
                {
		...
		public override void On非活性化()
		{
			...
			try
			{
				TJAPlayer4.ConfigIni.t書き出し( TJAPlayer4.strEXEのあるフォルダ + "Config.ini" );	// CONFIGだけ
		...
		public override int On進行描画()
		{
			...
			#region [ 背景 ]
			//---------------------
			if(TJAPlayer4.Tx.Config_Background != null )
                TJAPlayer4.Tx.Config_Background.t2D描画( TJAPlayer4.app.Device, 0, 0 );
			//---------------------
			#endregion
			#region [ メニューカーソル ]
			//---------------------
			if( TJAPlayer4.Tx.Config_Cursor != null )
			{
				Rectangle rectangle;
                TJAPlayer4.Tx.Config_Cursor.Opacity = this.bメニューにフォーカス中 ? 255 : 128;
				int x = 110;
				int y = (int)( 145.5 + ( this.n現在のメニュー番号 * 37.5 ) );
				int num3 = 340;
                TJAPlayer4.Tx.Config_Cursor.t2D描画( TJAPlayer4.app.Device, x, y, new Rectangle( 0, 0, 32, 48 ) );
                TJAPlayer4.Tx.Config_Cursor.t2D描画( TJAPlayer4.app.Device, ( x + num3 ) - 32, y, new Rectangle( 20, 0, 32, 48 ) );
				x += 32;
				for( num3 -= 64; num3 > 0; num3 -= rectangle.Width )
				{
                    ...
                    TJAPlayer4.Tx.Config_Cursor.t2D描画( TJAPlayer4.app.Device, x, y, rectangle );
					x += rectangle.Width;
				}
			}
			//---------------------
			...
			for ( int i = 0; i < txMenuItemLeft.GetLength( 0 ); i++ )
			{
                ...
                txMenuItemLeft[i, flag].t2D描画(TJAPlayer4.app.Device, 282 - (num4 / 2) + TJAPlayer4.Skin.Config_ItemText_Correction_X, menuY + TJAPlayer4.Skin.Config_ItemText_Correction_Y ); //55
				//txMenuItem.Dispose();
				menuY += stepY;
			}
			//---------------------
			...
			if( this.tx説明文パネル != null )
				this.tx説明文パネル.t2D描画( TJAPlayer4.app.Device, 67, 382 );
			//---------------------
			#endregion
			#region [ アイテム ]
			//---------------------
			...
			switch( base.eフェーズID )
			{
				case CStage.Eフェーズ.共通_フェードイン:
					...
					{
						TJAPlayer4.Skin.bgmコンフィグ画面.t再生する();
						base.eフェーズID = CStage.Eフェーズ.共通_通常状態;
					}
					break;
			...
			if ( !TJAPlayer4.EnumSongs.IsEnumerating || TJAPlayer4.actEnumSongs.bコマンドでの曲データ取得 != true )
			{
				if ( ( TJAPlayer4.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.Escape ) || TJAPlayer4.Pad.b押された( E楽器パート.DRUMS, Eパッド.FT ) ) || TJAPlayer4.Pad.b押されたGB( Eパッド.FT ) )
				{
				...
				else if ( ( TJAPlayer4.Pad.b押されたDGB( Eパッド.CY ) || TJAPlayer4.Pad.b押された( E楽器パート.DRUMS, Eパッド.RD ) ) || ( TJAPlayer4.Pad.b押された( E楽器パート.DRUMS, Eパッド.LC ) || ( TJAPlayer4.ConfigIni.bEnterがキー割り当てのどこにも使用されていない && TJAPlayer4.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.Return ) ) ) )
				{
		...
		private void t説明文パネルに現在選択されているメニューの説明を描画する()
		{
			try
			{
				...
				this.tx説明文パネル = new CTexture( TJAPlayer4.app.Device, image, TJAPlayer4.TextureFormat );
				image.Dispose();
			}
