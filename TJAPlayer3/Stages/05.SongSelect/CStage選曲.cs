...
namespace TJAPlayer4
{
    internal class CStage選曲 : CStage
    {
		...
		public void t選択曲変更通知()
		{
			...
			//---------------------
			if( TJAPlayer4.app != null )
			{
				var c曲リストノード = TJAPlayer4.stage選曲.r現在選択中の曲;
				var cスコア = TJAPlayer4.stage選曲.r現在選択中のスコア;

				if( c曲リストノード != null && cスコア != null && c曲リストノード.eノード種別 == C曲リストノード.Eノード種別.SCORE )
				{
					string str選択曲ファイル名 = cスコア.ファイル情報.ファイルの絶対パス;
					int n曲番号inブロック = TJAPlayer4.stage選曲.act曲リスト.n現在のアンカ難易度レベルに最も近い難易度レベルを返す(c曲リストノード, 0);

					foreach ( TJAPlayer4.STPlugin stPlugin in TJAPlayer4.app.listプラグイン )
					{
						Directory.SetCurrentDirectory( stPlugin.strプラグインフォルダ );
						stPlugin.plugin.On選択曲変更( str選択曲ファイル名, n曲番号inブロック );
						Directory.SetCurrentDirectory( TJAPlayer4.strEXEのあるフォルダ );
					}
				}
			}
			//---------------------
		...
		public override void On活性化()
		{
			...
			try
			{
				...
				for( int i = 0; i < 4; i++ )
					this.ctキー反復用[ i ] = new CCounter( 0, 0, 0, TJAPlayer4.Timer );
                ...
                // Discord Presenceの更新
                Discord.UpdatePresence("", Properties.Discord.Stage_SongSelect, TJAPlayer4.StartupTime);
            }
			finally
			{
                TJAPlayer4.ConfigIni.eScrollMode = EScrollMode.Normal;
                TJAPlayer4.ConfigIni.bスクロールモードを上書き = false;
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
                ...
                this.ct背景スクロール用タイマー = new CCounter(0, TJAPlayer4.Tx.SongSelect_Background.szテクスチャサイズ.Width, 30, TJAPlayer4.Timer);
		...
		public override int On進行描画()
		{
			...
			this.ct背景スクロール用タイマー.t進行Loop();
				...
				if( base.b初めての進行描画 )
				{
					this.ct登場時アニメ用共通 = new CCounter( 0, 100, 3, TJAPlayer4.Timer );
					if( TJAPlayer4.r直前のステージ == TJAPlayer4.stage結果 )
					{
				...
				if( TJAPlayer4.Tx.SongSelect_Background != null )
                    TJAPlayer4.Tx.SongSelect_Background.t2D描画( TJAPlayer4.app.Device, 0, 0 );
                ...
                if( this.r現在選択中の曲 != null )
                {
                    ...
                    if (TJAPlayer4.Tx.SongSelect_GenreBack[nGenreBack] != null )
                    {
                        for( int i = 0 ; i <(1280 / TJAPlayer4.Tx.SongSelect_Background.szテクスチャサイズ.Width) + 2; i++ )
                            if (TJAPlayer4.Tx.SongSelect_GenreBack[nGenreBack] != null )
                                    TJAPlayer4.Tx.SongSelect_GenreBack[nGenreBack].t2D描画(TJAPlayer4.app.Device, -ct背景スクロール用タイマー.n現在の値 + TJAPlayer4.Tx.SongSelect_Background.szテクスチャサイズ.Width * i , 0);
                    }
                }
			...
			//	this.bIsEnumeratingSongs = !this.actPreimageパネル.bIsPlayingPremovie;				// #27060 2011.3.2 yyagi: #PREMOVIE再生中は曲検索を中断する
                ...
                {
                    if (TJAPlayer4.Tx.NamePlate[i] != null)
                    {
                        TJAPlayer4.Tx.NamePlate[i].t2D描画(TJAPlayer4.app.Device, TJAPlayer4.Skin.SongSelect_NamePlate_X[i], TJAPlayer4.Skin.SongSelect_NamePlate_Y[i]);
                    }
                }
                #endregion

                #region[ 下部テキスト ]
                if (TJAPlayer4.Tx.SongSelect_Auto != null)
                {
					if (TJAPlayer4.ConfigIni.bAutoPlay[0])
					{
                        TJAPlayer4.Tx.SongSelect_Auto.t2D描画(TJAPlayer4.app.Device, TJAPlayer4.Skin.SongSelect_Auto_X[0], TJAPlayer4.Skin.SongSelect_Auto_Y[0]);
                    }
					if (TJAPlayer4.ConfigIni.nPlayerCount > 1 && TJAPlayer4.ConfigIni.bAutoPlay[1])
					{
                        TJAPlayer4.Tx.SongSelect_Auto.t2D描画(TJAPlayer4.app.Device, TJAPlayer4.Skin.SongSelect_Auto_X[1], TJAPlayer4.Skin.SongSelect_Auto_Y[1]);
                    }
                }
                if (TJAPlayer4.ConfigIni.eGameMode == EGame.完走叩ききりまショー)
                    TJAPlayer4.act文字コンソール.tPrint(0, 0, C文字コンソール.Eフォント種別.白, "GAME: SURVIVAL");
                if (TJAPlayer4.ConfigIni.eGameMode == EGame.完走叩ききりまショー激辛)
                    TJAPlayer4.act文字コンソール.tPrint(0, 0, C文字コンソール.Eフォント種別.白, "GAME: SURVIVAL HARD");
                if (TJAPlayer4.ConfigIni.bSuperHard)
                    TJAPlayer4.act文字コンソール.tPrint(0, 16, C文字コンソール.Eフォント種別.赤, "SUPER HARD MODE : ON");
                if (TJAPlayer4.ConfigIni.eScrollMode == EScrollMode.BMSCROLL)
                    TJAPlayer4.act文字コンソール.tPrint(0, 32, C文字コンソール.Eフォント種別.赤, "BMSCROLL : ON");
                else if (TJAPlayer4.ConfigIni.eScrollMode == EScrollMode.HBSCROLL)
                    TJAPlayer4.act文字コンソール.tPrint(0, 32, C文字コンソール.Eフォント種別.赤, "HBSCROLL : ON");
                #endregion
				...
				if (act曲リスト.bコース選択中[1])
					TJAPlayer4.Tx.SongSelect_Difficulty.t2D描画(TJAPlayer4.app.Device, 980, 30, new Rectangle(0, 70 * this.n現在選択中の曲の難易度[1], 260, 70));
				else if (act曲リスト.bコース選択中[0])
					TJAPlayer4.Tx.SongSelect_Difficulty.t2D描画(TJAPlayer4.app.Device, 980, 30, new Rectangle(0, 70 * this.n現在選択中の曲の難易度[0], 260, 70));


				if (!this.bBGM再生済み && (base.eフェーズID == CStage.Eフェーズ.共通_通常状態))
				{
					TJAPlayer4.Skin.bgm選曲画面.t再生する();
					this.bBGM再生済み = true;
				}
...
//Debug.WriteLine( "パンくず=" + this.r現在選択中の曲.strBreadcrumbs );
					...
					&& TJAPlayer4.act現在入力を占有中のプラグイン == null )
				{
					...
					{	// 詳細CONFIG呼び出し
						...
						TJAPlayer4.Skin.sound取消音.t再生する();
						return 0;
					}
					#endregion
					...
					{
						#region [ ESC BACK ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.Escape) && (this.act曲リスト.r現在選択中の曲 != null))
						{
						...
						#endregion
						#region [ F2  1P簡易オプション   ]
						if ( TJAPlayer4.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.F2 ) )
						{
						...
						if ( TJAPlayer4.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.F3 ) )
						{
							TJAPlayer4.Skin.sound変更音.t再生する();
							C共通.bToggleBoolian(ref TJAPlayer4.ConfigIni.bAutoPlay[0]);
						}
						#endregion
						#region [ F4 EMPTY ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F4))
						{
						}
						#endregion

						#region [ F5  EMPTY              ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F5))
						{
						}
						#endregion
						#region [ F6  2P簡易オプション   ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F6))
						{
							TJAPlayer4.Skin.sound変更音.t再生する();
						...
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F7))

						{
							if (TJAPlayer4.ConfigIni.nPlayerCount > 1)
                            {
                                TJAPlayer4.Skin.sound変更音.t再生する();
								C共通.bToggleBoolian(ref TJAPlayer4.ConfigIni.bAutoPlay[1]);
							}
                        }
						#endregion
						#region [ F8  EMPTY              ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F8))
						{
						}
						#endregion

						#region [ F9  スーパーハード     ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F9))
						{
							TJAPlayer4.Skin.sound変更音.t再生する();
                            C共通.bToggleBoolian( ref TJAPlayer4.ConfigIni.bSuperHard );
						}
						#endregion
						#region [ F10 SCROLL             ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F10))
						{
							TJAPlayer4.Skin.sound変更音.t再生する();
                            TJAPlayer4.ConfigIni.bスクロールモードを上書き = true;
                            switch( (int)TJAPlayer4.ConfigIni.eScrollMode )
                            {
                                case 0:
                                    TJAPlayer4.ConfigIni.eScrollMode = EScrollMode.BMSCROLL;
                                    break;
                                case 1:
                                    TJAPlayer4.ConfigIni.eScrollMode = EScrollMode.HBSCROLL;
                                    break;
                                case 2:
                                    TJAPlayer4.ConfigIni.eScrollMode = EScrollMode.Normal;
                                    TJAPlayer4.ConfigIni.bスクロールモードを上書き = false;
						...
						#endregion
						#region [ F11 EMPTY              ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F11))
						{
						}
						#endregion
						#region [ F12 EMPTY *割当非推進* ]
						if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.F12))
						{
						...
						{
                            #region [ Decide ]
                            if ((TJAPlayer4.Pad.b押されたDGB(Eパッド.Decide) || (TJAPlayer4.Pad.b押されたDGB(Eパッド.LRed) || TJAPlayer4.Pad.b押されたDGB(Eパッド.RRed)) ||
                                    ((TJAPlayer4.ConfigIni.bEnterがキー割り当てのどこにも使用されていない && TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.Return)))))
                            {
                                if (this.act曲リスト.r現在選択中の曲 != null)
                                {
                                    switch (this.act曲リスト.r現在選択中の曲.eノード種別)
                                    {
                                        case C曲リストノード.Eノード種別.SCORE:
											...
											else if (!act曲リスト.bコース選択中[1] && TJAPlayer4.ConfigIni.nPlayerCount == 2 && act曲リスト.n現在選択中の曲のコース数 > 1 && n現在選択中の曲の難易度[0] < 5)
											{
											...
											else
											{
												...
												if (TJAPlayer4.Skin.sound曲決定音.b読み込み成功)
													TJAPlayer4.Skin.sound曲決定音.t再生する();
												else
													TJAPlayer4.Skin.sound決定音.t再生する();
												this.t曲を選択する();
											}
							...
							{
								this.ctキー反復用.Up.tキー反復(TJAPlayer4.Input管理.Keyboard.bキーが押されている((int)SlimDX.DirectInput.Key.LeftArrow), new CCounter.DGキー処理(this.tカーソルを左へ移動する));
								...
								if (TJAPlayer4.Pad.b押された(E楽器パート.DRUMS, Eパッド.LBlue))
								{
									this.tカーソルを左へ移動する();
								}
							}
							...
							if (!act曲リスト.bコース選択中[0] && !act曲リスト.bコース選択中[1])
							{
								this.ctキー反復用.Down.tキー反復(TJAPlayer4.Input管理.Keyboard.bキーが押されている((int)SlimDX.DirectInput.Key.RightArrow), new CCounter.DGキー処理(this.tカーソルを右へ移動する));
								...
								if (TJAPlayer4.Pad.b押された(E楽器パート.DRUMS, Eパッド.RBlue))
								{
									this.tカーソルを右へ移動する();
								}
							}
							#endregion
							#region [ SPACE: 曲ソート ]
							if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.Space))
							{
								TJAPlayer4.Skin.sound変更音.t再生する();
							...
							#endregion
							#region [ 難易度変更(上) ]
							if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.RightArrow)
								|| TJAPlayer4.Pad.b押された(E楽器パート.DRUMS, Eパッド.RBlue))
							{
								if (act曲リスト.bコース選択中[1])
								{
							...
							#endregion
							#region [ 難易度変更(下) ]
							if (TJAPlayer4.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.LeftArrow)
								|| TJAPlayer4.Pad.b押された(E楽器パート.DRUMS, Eパッド.LBlue))
							{
								if (act曲リスト.bコース選択中[1])
								{
				    ...
				    KeyboardSoundGroupLevelControlHandler.Handle(
				        TJAPlayer4.Input管理.Keyboard, TJAPlayer4.SoundGroupLevelController, TJAPlayer4.Skin, true);
				    #endregion
...
//Debug.WriteLine("NOT start checking...stciCount=" + stciCount + ", targetCount=" + targetCount);
				...
				long curTime = TJAPlayer4.Timer.n現在時刻;
