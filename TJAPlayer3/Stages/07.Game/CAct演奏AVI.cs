...
namespace TJAPlayer4
{
	internal class CAct演奏AVI : CActivity
	{
		...
		{
            if ( ( nチャンネル番号 == 0x54 || nチャンネル番号 == 0x5A ) && TJAPlayer4.ConfigIni.bAVI有効 )
            {
                this.rAVI = rAVI;
                this.dsBGV = dsBGV;
                if (this.dsBGV != null && this.dsBGV.dshow != null)
                {
                    ...
                    if ( fAVIアスペクト比 < 1.77f )
                    {
                        ...
                        this.n移動開始時刻ms = (n移動開始時刻ms != -1) ? n移動開始時刻ms : (long)(CSound管理.rc演奏用タイマ.n現在時刻 * (((double)TJAPlayer4.ConfigIni.n演奏速度) / 20.0));
                    ...
                    if (this.tx描画用 == null)
                    {
                        this.tx描画用 = new CTexture(TJAPlayer4.app.Device, (int)this.framewidth, (int)this.frameheight, TJAPlayer4.app.GraphicsDeviceManager.CurrentSettings.BackBufferFormat, Pool.Managed);
                    }
		...
		{
			foreach ( CDTX.CChip chip in TJAPlayer4.DTX[0].listChip )
			{
		...
		public unsafe int t進行描画( int x, int y )
		{
			if ( !base.b活性化してない )
			{
				...
				int time = (int) ( ( CSound管理.rc演奏用タイマ.n現在時刻 - this.n移動開始時刻ms ) * ( ( (double) TJAPlayer4.ConfigIni.n演奏速度 ) / 20.0 ) );
				...
				if ((CSound管理.rc演奏用タイマ.n現在時刻 * (((double)TJAPlayer4.ConfigIni.n演奏速度) / 20.0)) < num4 )
				{
					num4 = (long)(CSound管理.rc演奏用タイマ.n現在時刻 * (((double)TJAPlayer4.ConfigIni.n演奏速度) / 20.0));
				}
				time = (int) ( ((CSound管理.rc演奏用タイマ.n現在時刻 * (((double)TJAPlayer4.ConfigIni.n演奏速度) / 20.0)) - num4 ));
				if ( num3 == 0 )
				{
                ...
                if( ( this.tx描画用 != null ))
                {
                    if ( ( this.bDShowクリップを再生している == true ) && this.dsBGV.dshow != null )
                    {
                        ...
                        if( TJAPlayer4.ConfigIni.eClipDispType == EClipDispType.背景のみ || TJAPlayer4.ConfigIni.eClipDispType == EClipDispType.両方 )
                        {
                            if( this.dsBGV.dshow.b上下反転 )
                                this.tx描画用.t2D上下反転描画( TJAPlayer4.app.Device, x, y );
                            else
                                this.tx描画用.t2D描画( TJAPlayer4.app.Device, x, y );
                        }
                        #endregion
                    }
                }
				else if ( ( this.tx描画用 != null ) && ( this.n総移動時間ms != -1 ) )
				{
                    ...
                    //とりあえず16:9以外は再生しない。
                    if( dbAVI比率 < 1.77 )
					    this.tx描画用.t2D描画( TJAPlayer4.app.Device, 0, 0 );
				}
        ...
        public void t窓表示()
        {
            ...
            if( ( this.bDShowクリップを再生している == true ) && this.dsBGV.dshow != null )
            {
                ...
                if( this.tx窓描画用 != null && fRet == 1.0 )
                {
                    ...
                    if( this.dsBGV.dshow.b上下反転 )
                        this.tx窓描画用.t2D上下反転描画( TJAPlayer4.app.Device, (int)fRatio[ 2 ], (int)fRatio[ 3 ] );
                    else
                        this.tx窓描画用.t2D描画( TJAPlayer4.app.Device, (int)fRatio[ 2 ], (int)fRatio[ 3 ] );
                }
