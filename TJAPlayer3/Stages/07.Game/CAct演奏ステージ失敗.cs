...
namespace TJAPlayer4
{
	internal class CAct演奏ステージ失敗 : CActivity
	{
		...
		public void Start()
		{
            this.dbFailedTime = TJAPlayer4.Timer.n現在時刻;
			this.ct進行 = new CCounter( 0, 1000, 2, TJAPlayer4.Timer );
            if( TJAPlayer4.ConfigIni.eGameMode != EGame.OFF )
            {
			    this.ct進行 = new CCounter( 0, 4000, 2, TJAPlayer4.Timer );
            }
		}
		...
		public override void On非活性化()
		{
			...
			if( this.sd効果音 != null )
			{
				TJAPlayer4.Sound管理.tサウンドを破棄する( this.sd効果音 );
				this.sd効果音 = null;
			}
			base.On非活性化();
		}
    ...
    //            CDTXMania.tテクスチャの解放( ref this.tx数字 );
		...
		public override int On進行描画()
		{
            ...
            if (TJAPlayer4.ConfigIni.eGameMode == EGame.完走叩ききりまショー || TJAPlayer4.ConfigIni.eGameMode == EGame.完走叩ききりまショー激辛)
            {
                ...
                {
                    if (TJAPlayer4.Tx.Failed_Game != null)
                        TJAPlayer4.Tx.Failed_Game.t2D描画(TJAPlayer4.app.Device, 0, 0);
            ...
            else
            {
                if (this.ct進行.n現在の値 < 100)
                {
                    ...
                    if ((x != 640) && (TJAPlayer4.Tx.Failed_Stage != null))
                    {
                        TJAPlayer4.Tx.Failed_Stage.t2D描画(TJAPlayer4.app.Device, 0, 0, new Rectangle(x, 0, 640 - x, 720));
                        TJAPlayer4.Tx.Failed_Stage.t2D描画(TJAPlayer4.app.Device, 640 + x, 0, new Rectangle(640, 0, 640 - x, 720));
                    }
                }
                else
                {
                    if (TJAPlayer4.Tx.Failed_Stage != null)
                    {
                        TJAPlayer4.Tx.Failed_Stage.t2D描画(TJAPlayer4.app.Device, 0, 0);
                    }
                    if (this.ct進行.n現在の値 <= 250)
                    {
                        int num2 = TJAPlayer4.Random.Next(5) - 2;
                        int y = TJAPlayer4.Random.Next(5) - 2;
                        if (TJAPlayer4.Tx.Failed_Stage != null)
                        {
                            TJAPlayer4.Tx.Failed_Stage.t2D描画(TJAPlayer4.app.Device, num2, y);
                        }
                    }
                    if (!this.b効果音再生済み)
                    {
                        TJAPlayer4.Skin.soundSTAGEFAILED音.t再生する();
                        this.b効果音再生済み = true;
                    }
                }
            }
			...
			foreach( char ch in str )
			{
				for( int i = 0; i < this.st文字位置.Length; i++ )
				{
					if( this.st文字位置[ i ].ch == ch )
					{
						...
						{
                            TJAPlayer4.Tx.Balloon_Number_Roll.t2D描画( TJAPlayer4.app.Device, x - ( 62 * str.Length / 2 ), y, rectangle );
						}
						break;
					}
				}
