...
namespace TJAPlayer4
{
	internal class Rainbow : CActivity
	{
		...
		{
            if (TJAPlayer4.Tx.Effects_Rainbow != null)
            {
                for (int i = 0; i < 2; i++)
                {
                    if (!this.Rainbow1P[i].IsUsing && player == 0)
                    {
                        ...
                        this.Rainbow1P[i].Counter = new CCounter(0, 164, TJAPlayer4.Skin.Game_Effect_Rainbow_Timer, TJAPlayer4.Timer); // カウンタ
                    ...
                    if (!this.Rainbow2P[i].IsUsing && player == 1)
                    {
                        ...
                        this.Rainbow2P[i].Counter = new CCounter(0, 164, TJAPlayer4.Skin.Game_Effect_Rainbow_Timer, TJAPlayer4.Timer); // カウンタ
			...
			if( !base.b活性化してない )
			{
                for (int f = 0; f < 2; f++)
                {
                    if (this.Rainbow1P[f].IsUsing)
                    {
                        ...
                        if(TJAPlayer4.Tx.Effects_Rainbow != null && this.Rainbow1P[f].Player == 0 ) //画像が出来るまで
                        {
                            ...
                            if (this.Rainbow1P[f].Counter.n現在の値 < 82)
                            {
                                ...
                                TJAPlayer4.Tx.Effects_Rainbow.t2D描画(TJAPlayer4.app.Device, 360, -100, new Rectangle(0, 0, nRectX, 410));
                            }
                            else if (this.Rainbow1P[f].Counter.n現在の値 >= 82)
                            {
                                int nRectX = (((this.Rainbow1P[f].Counter.n現在の値 - 82) * 920) / 85);
                                TJAPlayer4.Tx.Effects_Rainbow.t2D描画(TJAPlayer4.app.Device, 360 + nRectX, -100, new Rectangle(nRectX, 0, 920 - nRectX, 410));
                            }
                ...
                for (int f = 0; f < 2; f++)
                {
                    if (this.Rainbow2P[f].IsUsing)
                    {
                        ...
                        if(TJAPlayer4.Tx.Effects_Rainbow != null && this.Rainbow2P[f].Player == 1 ) //画像が出来るまで
                        {
                            //this.st虹[f].ct進行.n現在の値 = 164;
                            ...
                            if (this.Rainbow2P[f].Counter.n現在の値 < 82)
                            {
                                ...
                                TJAPlayer4.Tx.Effects_Rainbow.t2D上下反転描画(TJAPlayer4.app.Device, 360, 410, new Rectangle(0, 0, nRectX, 410));
                            }
                            else if (this.Rainbow2P[f].Counter.n現在の値 >= 82)
                            {
                                int nRectX = (((this.Rainbow2P[f].Counter.n現在の値 - 82) * 920) / 85);
                                TJAPlayer4.Tx.Effects_Rainbow.t2D上下反転描画(TJAPlayer4.app.Device, 360 + nRectX, 410, new Rectangle(nRectX, 0, 920 - nRectX, 410));
                            }
